...
import static vitae.org.vitaewallet.ui.transaction_detail_activity.FragmentTxDetail.IS_DETAIL;
import static vitae.org.vitaewallet.ui.transaction_detail_activity.FragmentTxDetail.TX_WRAPPER;
import static vitae.org.vitaewallet.utils.TxUtils.getAddressOrContact;
...
public class TransactionsFragmentBase extends BaseRecyclerFragment<TransactionWrapper> {
    ...
    protected List<TransactionWrapper> onLoading() {
        List<TransactionWrapper> list = vitaeModule.listTx();
    ...
    protected BaseRecyclerAdapter<TransactionWrapper, ? extends BaseRecyclerViewHolder> initAdapter() {
        BaseRecyclerAdapter<TransactionWrapper, TransactionViewHolderBase> adapter = new BaseRecyclerAdapter<TransactionWrapper, TransactionViewHolderBase>(getActivity()) {
            ...
            protected void bindHolder(TransactionViewHolderBase holder, TransactionWrapper data, int position) {
                ...
                if (vitaeRate!=null) {
                    localCurrency = vitaeApplication.getCentralFormats().format(
                                    new BigDecimal(data.getAmount().getValue() * vitaeRate.getValue().doubleValue()).movePointLeft(8)
                                    )
                                    + " " + vitaeRate.getCoin();
                ...
                holder.title.setText(getAddressOrContact(vitaeModule,data));
                ...
                /*if (data.getOutputLabels()!=null && !data.getOutputLabels().isEmpty()){
                    ...
                    if (contact!=null) {
                        ...
                        holder.title.setText(data.getTransaction().getOutput(0).getScriptPubKey().getToAddress(vitaeModule.getConf().getNetworkParams()).toBase58());
                    }
                }else {
                    holder.title.setText(data.getTransaction().getOutput(0).getScriptPubKey().getToAddress(vitaeModule.getConf().getNetworkParams()).toBase58());
                }*/
