package vitae.org.vitaewallet.ui.pincode_activity;
...
import vitae.org.vitaewallet.ui.backup_mnemonic_activity.MnemonicActivity;
import vitae.org.vitaewallet.ui.base.BaseActivity;
import vitae.org.vitaewallet.ui.settings_pincode_activity.KeyboardFragment;
import vitae.org.vitaewallet.ui.start_activity.StartActivity;

import static vitae.org.vitaewallet.ui.backup_mnemonic_activity.MnemonicActivity.INTENT_EXTRA_INIT_VIEW;
...
public class PincodeActivity extends BaseActivity implements KeyboardFragment.onKeyListener {
    ...
    protected void onCreateView(Bundle savedInstanceState, ViewGroup container) {
        ...
        if (vitaeApplication.getAppConf().getPincode()!=null && !checkPin){
            goNext();
            finish();
        }
    ...
    private void goNext() {
        if (vitaeApplication.getAppConf().getTrustedNode()==null){
            // select random trusted node
            List<BwktrumPeerData> nodes = BwktrumGlobalData.listTrustedHosts();
            Random random = new Random();
            vitaeApplication.setTrustedServer(nodes.get(random.nextInt(nodes.size())));
            vitaeApplication.stopBlockchain();
        }

        vitaeApplication.getAppConf().setAppInit(true);
        ...
        if (lastPos<4) {
            if (key.getValue() < 10) {
                ...
                if (lastPos == 4) {
                    ...
                    if (!checkPin) {
                        vitaeApplication.getAppConf().savePincode(pincode);
                        Toast.makeText(this, R.string.pincode_saved, Toast.LENGTH_SHORT).show();
                        goNext();
                    }else {
                        // check pin and return result
                        if(vitaeApplication.getAppConf().getPincode().equals(pincode)){
