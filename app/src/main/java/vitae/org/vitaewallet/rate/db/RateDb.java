package vitae.org.vitaewallet.rate.db;
...
import vitae.org.vitaewallet.contacts.AbstractSqliteDb;

/**
 * Created by kaali on 7/5/17.
 */

public class RateDb extends AbstractSqliteDb<VitaeRate> {
    ...
    @Override
    protected ContentValues buildContent(VitaeRate obj) {
    ...
    @Override
    protected VitaeRate buildFrom(Cursor cursor) {
        ...
        return new VitaeRate(coin,value,timestap,link);
    }
    ...
    public void insertOrUpdateIfExist(VitaeRate vitaeRate) {
        if (getRate(vitaeRate.getCoin())==null){
            insert(vitaeRate);
        }else {
            updateByKey(KEY_COIN,vitaeRate.getCoin(),vitaeRate);
        }
    }
}
