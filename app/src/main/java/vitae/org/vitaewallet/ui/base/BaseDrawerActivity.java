...
import vitae.org.vitaewallet.ui.Exchange;
import vitae.org.vitaewallet.ui.contacts_activity.ContactsActivity;
import vitae.org.vitaewallet.ui.donate.DonateActivity;
import vitae.org.vitaewallet.ui.settings_activity.SettingsActivity;
import vitae.org.vitaewallet.ui.wallet_activity.WalletActivity;

import static vitae.org.vitaewallet.module.VitaeContext.OUT_OF_SYNC_TIME;
...
public class BaseDrawerActivity extends VitaeActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    private void checkState(){
        ...
        long lastBlockTime = vitaeApplication.getAppConf().getLastBestChainBlockTime();
        if (lastBlockTime+OUT_OF_SYNC_TIME>now){
            // check if i'm syncing or i'm synched
            long peerHeight = vitaeModule.getConnectedPeerHeight();
            if (peerHeight!=-1){
                if (vitaeModule.getChainHeight()+10>peerHeight) {
            ...
            if (vitaeModule.isAnyPeerConnected()) {
                long peerHeight = vitaeModule.getConnectedPeerHeight();
                if (peerHeight!=-1){
                    if (vitaeModule.getChainHeight()+10>peerHeight) {
                        blockchainState = BlockchainState.SYNC;
    ...
    private double calculateBlockchainSyncProgress() {
        long nodeHeight = vitaeModule.getConnectedPeerHeight();
        if (nodeHeight>0){
            // calculate the progress
            // nodeHeight -> 100 %
            // current height -> x %
            return (vitaeModule.getChainHeight()*100) / nodeHeight;
        }
        return -1;
    }
